...
from basic_ssvm import BaseSSVM
from basic_inference import loss_augmented_inference
...
class OneSlackSSVM(BaseSSVM):
    ...
    def __init__(self, model, max_iter=10000, C=1.0, check_constraints=False,
                 ...
                 switch_to=None, pooling=None, log = None):
    ...
    def writeLog(self, string, toconsole = False):
    ...
    def _solve_1_slack_qp(self, constraints, n_samples):
        ...
        #print("joint_features: {}".format(len(joint_features)))
        losses = [c[1] for c in constraints]
        nonzerow=len(self.w)
        #nonzerow=0

        n_constraints = len(joint_features)+nonzerow
        if nonzerow==0:
            joint_feature_matrix = np.vstack(joint_features)
        else:
            joint_feature_matrix = np.vstack((joint_features,np.identity(nonzerow)))
            
        
        #print("joint feature_matrix: {}".format(joint_feature_matrix.shape))
        
        P = cvxopt.matrix(np.dot(joint_feature_matrix, joint_feature_matrix.T))
        # q contains loss from margin-rescaling
        #print(np.concatenate([-np.array(losses, dtype=np.float),np.zeros(len(self.w))]))
        #input("Press Enter to continue...")
        if nonzerow==0:
            q = cvxopt.matrix(-np.array(losses, dtype=np.float))
        else:
            q = cvxopt.matrix(np.concatenate([-np.array(losses, dtype=np.float),np.zeros(nonzerow)]))
        
        # constraints: all alpha must be >zero
        idy = np.identity(n_constraints)
        tmp1 = np.zeros(n_constraints)
        # constraints: all weights must be >= zero
        #w_idy = np.identity(n_constraints)
        #w_tmp1 = np.zeros(n_constraints)
        ...
        a1 =np.ones(len(joint_features))
        a2 =np.zeros(nonzerow)
        #print(a1)
        #print(a2)
        #np.concatenate([a1,a2])
        if nonzerow==0:
            A = cvxopt.matrix(np.ones((1,len(joint_features))))
        else:
            A = cvxopt.matrix(np.asmatrix([np.concatenate([a1,a2])]))        
    ...
    def _find_new_constraint(self, X, Y, joint_feature_gt, constraints, check=False):
        self.writeLog("self._find_new_constraint RUNNING", toconsole = True)
        #print("self._find_new_constraint RUNNING")
        Y_hat=self.model.batch_loss_augmented_inference(X, Y, self.w, n_jobs=self.n_jobs)   
        #print(Y_hat)
        #sys.exit();
        ...
        if getattr(self.model, 'rescale_C', False):
            ...
            joint_feature= self.model.batch_joint_feature(X, Y_hat, n_jobs=self.n_jobs)
            djoint_feature = (joint_feature_gt - joint_feature) / len(X)
            #print("here")
            #print(djoint_feature)
            #print(joint_feature_gt)

        loss_mean = np.mean(self.model.batch_loss(Y, Y_hat))

        #print(self.w)
        #count = 0
        w_dfeature = np.dot(self.w, djoint_feature)
        #for i in w_dfeature.tolist():
        #   if i>0:
        #        count += 1
        print("w.dfeature: {}".format(w_dfeature))
        violation = loss_mean - np.dot(self.w, djoint_feature)
        ...
        self.writeLog("self._find_new_constraint Done", toconsole = True)
        #print("self._find_new_constraint Done")
        return Y_hat, djoint_feature, loss_mean
    ...
    def fit(self, X, Y, constraints=None, warm_start=False, initialize=True):
        ...
        if self.log is not None:
            self.writeLog("Training 1-slack dual structural SVM", toconsole = True)
        ...
        if not warm_start:
            ...
            self.w = np.random.rand(self.model.size_joint_feature)
            constraints = []
        ...
        if getattr(self.model, 'rescale_C', False):
            ...
            joint_feature_gt = self.model.batch_joint_feature(X, Y, n_jobs=self.n_jobs)
        ...
        try:
            ...
            for iteration in range(self.max_iter):
                ...
                if self.log is not None:
                    self.writeLog("iteration " + str(iteration),toconsole = True)
                ...
                try:
                    ...
                    try:
                        ...
                        if (self.switch_to is not None
                            ...
                            print("break due to constraint too weak")
                            break
                if self.log is not None:
                    self.writeLog("constrains retrieved ",toconsole = True)
                ...
                if self.log is not None:
                    self.writeLog("computing primal objective",toconsole = True)
                ...
                if primal_objective < best_objective:
                    best_w = self.w
                ...
                if self.log is not None:
                    self.writeLog("updating cache tolerance if cache_tol is auto",toconsole = True)
                ...
                if self.log is not None:
                    self.writeLog("cutting plane objective: {}, primal objective {}".format(objective, primal_objective),toconsole = True)
                ...
                if True:
                    ...
                    for i in range(len(self.w)):
                        if self.w[i]>0:
                            ...
                            self.w[i]=0
                    #new_w=[]
                    self.model.instance.featureIndicator_reset(0)
                    for i in indexList:
                        #new_w.append(self.w[i])
                        self.model.instance.featureIndicator[i]=1
                    #self.model.instance.featureNum=len(new_w)
                    #self.model.size_joint_feature=len(new_w)
                    print(self.model.instance.featureNum)
                    #self.w=np.array(new_w)
                    #self.model.instance.diffusionGraphs=new_diffusionGraphs
        ...
        if self.log is not None:
                    self.writeLog("computing final objective",toconsole = True)
        ...
        if self.log is not None:
                    self.writeLog("computing final objective Done",toconsole = True)
