...
class TestDataset(torch.utils.data.Dataset):
    def __init__(self, pair_path, size, batch_size, lineNums , topic_size, node_size, train=True, full=False, max_size=None):
        self.train = train
        self.full = full
        self.size=size
        self.max_size=max_size
        self.data = self.cache(pair_path, lineNums, topic_size, node_size,)
        #self.max = 20
        self.batch_size=batch_size
        

    def cache(self, pair_path,lineNums,topic_size, node_size):
        ...
        for datapoint in sample_data:
            ...
            set_train = one_hot_encoder(np.array(set_train), topic_size ,max_size).transpose()
            #print("set_train.shape {}".format(set_train.shape))

            #set_train = [np.array(set_train).transpose()]
            #print(set_train.shape)
            label = one_hot_encoder(np.array(label), node_size, max_size).transpose()
...
def extract_training_sets(filename, size,lineNums):
    content = load_file(filename, size, lineNums)
...
def build_net(args):
    set_channels_input = args.topic_size
    set_channels_output = args.node_size
    ...
    set_encoder = MLPEncoderInput(set_channels_output, output_channels, set_size, set_encoder_dim)

    set_decoder = DSPN(set_encoder, set_channels_output, set_size, iters, inner_lr,args.batch_size)

    
    input_encoder = MLPEncoderInput(set_channels_input, output_channels, set_size, input_encoder_dim)
